# 🚀 VLA Learning Journey
> My journey learning Vision-Language-Action models from scratch to research level

[![GitHub stars](https://img.shields.io/github/stars/1YundaWang/VLA-Learning?style=social)](https://github.com/1YundaWang/VLA-Learning)
[![License](https://img.shields.io/badge/license-MIT-blue.svg)](LICENSE)

## 📖 About this Repository

This is the complete record of my Vision–Language–Action (VLA) learning journey, starting February 2026.

**Background:** Strong foundation in Python and deep learning; goal is to master VLA techniques and publish research.

**Learning Path:** Systematic learning plan based on the StarVLA project

**Update Frequency:** At least 3 commits per week

## 🎯 Learning Goals

- [ ] **Month 1:** Master modern architectures such as Transformer and ViT
- [ ] **Month 2:** Understand VLA implementation details
- [ ] **Months 3–4:** Complete MVP research project

## 📚 Progress

### ✅ Completed
- [√] Environment setup and StarVLA deployment
- [√] Learning roadmap finalized

### 🔄 In Progress
- [ ] Week 1: Transformer architecture study
    - [ ] Read "Attention is All You Need"
    - [ ] Implement multi-head attention
    - [ ] Implement self-attention from scratch
    - [ ] LayerNorm and residual connections
    - [ ] Build a mini-Transformer
    - [ ] Understand positional encoding

### 📅 Planned
- [ ] Week 2: Vision Transformer
- [ ] Week 3: Run StarVLA evaluation
- [ ] Week 4: Understand data flow

## 📁 Repository Structure
```
├── notes/           # Study notes and paper summaries
├── code/            # Code implementations and experiments
├── experiments/     # Experimental results and logs
└── resources/       # Learning resources
```

## 🔬 Experiment Log

| ID | Experiment Name | Date | Status | Key Findings |
|----|-----------------|------|--------|--------------|
| 001 |  |  |  |

## 🛠️ Tech Stack
- **Deep Learning Framework:** PyTorch 2.0+
- **VLA Project:** StarVLA
- **Experiment Tracking:** Weights & Biases
- **Version Control:** Git + GitHub

## 📖 Recommended Resources

### Essential Papers
1. [name](link)

### Courses and Tutorials
- [name](link)

## 🤝 Feedback and Discussion
- **Issues:** Welcome for questions and suggestions
- **Discussions:** Share learning insights
- **Email:** 2329033510@qq.com

## 📄 License
This project is licensed under the [MIT License](LICENSE).

## 🙏 Acknowledgments

- [StarVLA](https://github.com/starVLA/starVLA) - Main learning project
- [OpenVLA](https://github.com/openvla/openvla) - Reference implementation
- All open-source community members who helped

---

⭐ If this repository helps you, please consider giving it a star!

**Last Updated:** 2026-02-06